 ...
 * unidirectional constant shear. An artifical CM1 grid with no 
...
/* Create a constant unidirectional shear profile
 * to integrate the trajectories through. This is
 * done by specifying a constant shear with height
 * and then changing U or V based on that shear. */
void create_shear(datagrid *grid, integration_data *data) {
    int NX = grid->NX;
    int NY = grid->NY;
    int NZ = grid->NZ;
    float shear = 0.02;
    ...
    for (int i = 0; i < NX+1; ++i) {
        for (int j = 0; j < NY; ++j) {
            for (int k = 0; k < NZ; ++k) {
                for (int t = 0; t < 4; ++t) {
                    data->u_4d_chunk[P4(i, j, k, t, NX, NY, NZ)] = u0 + shear*grid->zh[k]; 
                }
            }
        }
    }
    ...
    for (int i = 0; i < NX; ++i) {
        for (int j = 0; j < NY+1; ++j) {
            for (int k = 0; k < NZ; ++k) {
                for (int t = 0; t < 4; ++t) {
                    data->v_4d_chunk[P4(i, j, k, t, NX, NY, NZ)] = v0 + shear*grid->zh[k]; 
                }
            }
        }
    } 
    ...
    for (int i = 0; i < NX+1; ++i) {
        for (int j = 0; j < NY+1; ++j) {
            for (int k = 0; k < NZ+1; ++k) {
                for (int t = 0; t < 4; ++t) {
                    data->w_4d_chunk[P4(i, j, k, t, NX, NY, NZ)] = 0.0; // m/s
                }
            }
        }
    } 
...
int main(int argc, char **argv ) {
    ...
    string outfilename = string("uni_shear") + ".nc";
